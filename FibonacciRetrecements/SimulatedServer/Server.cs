...
namespace SimulatedServer
{
    public class Server
    {
        ...
        public List<CodeParameterBase> CodeParameters { get; private set; }  //parameters of your code (will be added to this collection  automatically)
        ...
        public void Stop()
        {
            if (Code != null && Code is SignalBase)
                ((SignalBase)Code).SetSignalState(SignalState.Stopped);
        ...
        private void CreateFibonacciRetrecements()
        {
            // Auto generated code
            Code = new global::FibonacciRetrecements.FibonacciRetrecements();
        ...
        private void OnNewBarAppended(object sender, HistoricalData data)
        {
            try
            {
                if (Code is SignalBase)
                {
                    ...
                    RaiseOutput(String.Format(" > On new bar start, time: {0}", seconds.ToString("g")));
                    ...
                    {
                        foreach (var alert in alerts)
                            RaiseOutput(String.Format(" - Alert: {0}", alert));
                    }
                }
            }
            catch (Exception ex)
        ...
        private void RunOnTimer(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                signal.Start();
                var seconds = (DateTime.UtcNow - _prevExecution);
                RaiseOutput(String.Format(" > Periodic signal execution, time: {0}", seconds.ToString("g")));
        ...
        private void BacktestStatusCheck(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (signal != null)
            {
                ...
                if (signal.BacktestProgress > _backtestProgress + 5F)
                    RaiseOutput($" > Backtest progress: {signal.BacktestProgress:0}%");
                ...
                if (_backtestProgress >= 100F)
                {
                    ...
                    RaiseOutput(" > Finished backtest: " + Environment.NewLine + summary.ToString());
